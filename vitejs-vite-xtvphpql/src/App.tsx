import { useEffect, useMemo, useState } from 'react';
import { supabase } from './supabaseClient';
import './index.css';

type UserProfile = {
  id: string;
  email: string;
  can_consult: boolean;
};

type Registro = {
  id: number;
  user_id: string;
  numero_cracha: string;
  nome: string;
  box_acautelou: string;
  horario_entrada: string;
  horario_saida: string | null;
  created_at: string;
};

type CrachasAutorizados = {
  numero_cracha: string;
  nome_referencia: string | null;
  ativo: boolean;
};

export default function App() {
  const [sessionLoading, setSessionLoading] = useState(true);
  const [userId, setUserId] = useState<string | null>(null);
  const [profile, setProfile] = useState<UserProfile | null>(null);

  const [modoAuth, setModoAuth] = useState<'login' | 'cadastro'>('login');
  const [email, setEmail] = useState('');
  const [senha, setSenha] = useState('');

  const [numeroCracha, setNumeroCracha] = useState('');
  const [nome, setNome] = useState('');
  const [boxAcautelou, setBoxAcautelou] = useState('');
  const [horarioEntrada, setHorarioEntrada] = useState('');
  const [horarioSaida, setHorarioSaida] = useState('');

  const [buscaNome, setBuscaNome] = useState('');
  const [buscaData, setBuscaData] = useState('');

  const [registros, setRegistros] = useState<Registro[]>([]);
  const [crachasAutorizados, setCrachasAutorizados] = useState<string[]>([]);
  const [loadingRegistros, setLoadingRegistros] = useState(false);

  useEffect(() => {
    const init = async () => {
      const {
        data: { session },
      } = await supabase.auth.getSession();

      const currentUser = session?.user ?? null;
      setUserId(currentUser?.id ?? null);

      if (currentUser?.id) {
        await carregarProfile(currentUser.id);
        await carregarCrachas();
        await carregarRegistros();
      }

      setSessionLoading(false);
    };

    init();

    const {
      data: { subscription },
    } = supabase.auth.onAuthStateChange(async (_event, session) => {
      const currentUser = session?.user ?? null;
      setUserId(currentUser?.id ?? null);

      if (currentUser?.id) {
        await carregarProfile(currentUser.id);
        await carregarCrachas();
        await carregarRegistros();
      } else {
        setProfile(null);
        setRegistros([]);
      }

      setSessionLoading(false);
    });

    return () => {
      subscription.unsubscribe();
    };
  }, []);

  async function carregarProfile(uid: string) {
    const { data, error } = await supabase
      .from('profiles')
      .select('id, email, can_consult')
      .eq('id', uid)
      .single();

    if (error) {
      console.error(error);
      return;
    }

    setProfile(data);
  }

  async function carregarCrachas() {
    const { data, error } = await supabase
      .from('crachas_autorizados')
      .select('numero_cracha, nome_referencia, ativo')
      .eq('ativo', true)
      .order('numero_cracha', { ascending: true });

    if (error) {
      console.error(error);
      return;
    }

    setCrachasAutorizados(
      (data as CrachasAutorizados[]).map((item) => item.numero_cracha)
    );
  }

  async function carregarRegistros() {
    setLoadingRegistros(true);

    const { data, error } = await supabase
      .from('registros')
      .select('*')
      .order('horario_entrada', { ascending: false });

    setLoadingRegistros(false);

    if (error) {
      console.error(error);
      alert('Erro ao carregar registros.');
      return;
    }

    setRegistros(data as Registro[]);
  }

  async function cadastrar() {
    if (!email || !senha) {
      alert('Preencha e-mail e senha.');
      return;
    }

    const { error } = await supabase.auth.signUp({
      email,
      password: senha,
    });

    if (error) {
      alert(error.message);
      return;
    }

    alert('Cadastro realizado. Agora faça login.');
    setModoAuth('login');
  }

  async function login() {
    if (!email || !senha) {
      alert('Preencha e-mail e senha.');
      return;
    }

    const { error } = await supabase.auth.signInWithPassword({
      email,
      password: senha,
    });

    if (error) {
      alert(error.message);
    }
  }

  async function sair() {
    await supabase.auth.signOut();
  }

  function limparFormulario() {
    setNumeroCracha('');
    setNome('');
    setBoxAcautelou('');
    setHorarioEntrada('');
    setHorarioSaida('');
  }

  async function salvarEntradaOuSaida() {
    if (!userId) {
      alert('Usuário não autenticado.');
      return;
    }

    if (!numeroCracha || !nome || !boxAcautelou || !horarioEntrada) {
      alert('Preencha nº do crachá, nome, box acautelou e horário de entrada.');
      return;
    }

    if (!crachasAutorizados.includes(numeroCracha.trim().toUpperCase())) {
      alert('Esse crachá não está na lista autorizada da ESMPDFT.');
      return;
    }

    const crachaNormalizado = numeroCracha.trim().toUpperCase();

    if (!horarioSaida) {
      const { data: abertoNoDia, error: erroBusca } = await supabase
        .from('registros')
        .select('*')
        .eq('numero_cracha', crachaNormalizado)
        .is('horario_saida', null)
        .order('horario_entrada', { ascending: false })
        .limit(1);

      if (erroBusca) {
        alert('Erro ao verificar registro em aberto.');
        return;
      }

      if (abertoNoDia && abertoNoDia.length > 0) {
        alert('Já existe uma entrada em aberto para esse crachá.');
        return;
      }

      const { error } = await supabase.from('registros').insert({
        user_id: userId,
        numero_cracha: crachaNormalizado,
        nome,
        box_acautelou: boxAcautelou,
        horario_entrada: horarioEntrada,
        horario_saida: null,
      });

      if (error) {
        alert('Erro ao salvar entrada.');
        return;
      }

      alert('Entrada registrada com sucesso.');
      limparFormulario();
      await carregarRegistros();
      return;
    }

    const { data: aberto, error: erroAberto } = await supabase
      .from('registros')
      .select('*')
      .eq('numero_cracha', crachaNormalizado)
      .is('horario_saida', null)
      .order('horario_entrada', { ascending: false })
      .limit(1);

    if (erroAberto) {
      alert('Erro ao localizar registro em aberto.');
      return;
    }

    if (!aberto || aberto.length === 0) {
      alert('Não existe registro em aberto para esse crachá.');
      return;
    }

    const registroAberto = aberto[0] as Registro;

    const { error: erroUpdate } = await supabase
      .from('registros')
      .update({
        horario_saida: horarioSaida,
      })
      .eq('id', registroAberto.id);

    if (erroUpdate) {
      alert('Erro ao registrar saída.');
      return;
    }

    alert('Saída registrada com sucesso.');
    limparFormulario();
    await carregarRegistros();
  }

  const registrosFiltrados = useMemo(() => {
    return registros.filter((item) => {
      const bateNome =
        !buscaNome ||
        item.nome.toLowerCase().includes(buscaNome.trim().toLowerCase());

      const dataEntrada = item.horario_entrada?.slice(0, 10);
      const bateData = !buscaData || dataEntrada === buscaData;

      return bateNome && bateData;
    });
  }, [registros, buscaNome, buscaData]);

  if (sessionLoading) {
    return (
      <div className="page center-screen">
        <div className="card auth-card">
          <h1>Carregando...</h1>
        </div>
      </div>
    );
  }

  if (!userId) {
    return (
      <div className="page auth-page">
        <div className="auth-wrapper">
          <div className="brand-panel">
            <h1>Controle de Entrada e Saída</h1>
            <p>
              Sistema privado para controle de crachás ESMPDFT com login,
              consulta e registros em aberto.
            </p>
          </div>

          <div className="card auth-card">
            <h2>{modoAuth === 'login' ? 'Entrar' : 'Cadastrar'}</h2>

            <div className="field">
              <label>E-mail</label>
              <input
                type="email"
                placeholder="seuemail@dominio.com"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
              />
            </div>

            <div className="field">
              <label>Senha</label>
              <input
                type="password"
                placeholder="Digite sua senha"
                value={senha}
                onChange={(e) => setSenha(e.target.value)}
              />
            </div>

            <div className="auth-actions">
              {modoAuth === 'login' ? (
                <>
                  <button className="btn-primary" onClick={login}>
                    Entrar
                  </button>
                  <button
                    className="btn-secondary"
                    onClick={() => setModoAuth('cadastro')}
                  >
                    Criar conta
                  </button>
                </>
              ) : (
                <>
                  <button className="btn-primary" onClick={cadastrar}>
                    Cadastrar
                  </button>
                  <button
                    className="btn-secondary"
                    onClick={() => setModoAuth('login')}
                  >
                    Voltar ao login
                  </button>
                </>
              )}
            </div>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="page app-bg">
      <div className="app-shell">
        <header className="topbar">
          <div>
            <h1>Controle de Entrada e Saída</h1>
            <p className="subtitle">
              Usuário: {profile?.email || 'sem e-mail'}{' '}
              {profile?.can_consult ? '• Perfil com consulta' : ''}
            </p>
          </div>

          <button className="btn-secondary" onClick={sair}>
            Sair
          </button>
        </header>

        <section className="card">
          <h2>Lançamento</h2>

          <div className="form-grid">
            <div className="field">
              <label>N° do Crachá</label>
              <input
                type="text"
                placeholder="Ex.: ESM001"
                value={numeroCracha}
                onChange={(e) => setNumeroCracha(e.target.value.toUpperCase())}
              />
            </div>

            <div className="field">
              <label>Nome</label>
              <input
                type="text"
                placeholder="Digite o nome"
                value={nome}
                onChange={(e) => setNome(e.target.value)}
              />
            </div>

            <div className="field">
              <label>Box Acautelou</label>
              <input
                type="text"
                placeholder="Ex.: Box 03"
                value={boxAcautelou}
                onChange={(e) => setBoxAcautelou(e.target.value)}
              />
            </div>

            <div className="field">
              <label>Horário de entrada</label>
              <input
                type="datetime-local"
                value={horarioEntrada}
                onChange={(e) => setHorarioEntrada(e.target.value)}
              />
            </div>

            <div className="field">
              <label>Horário de saída</label>
              <input
                type="datetime-local"
                value={horarioSaida}
                onChange={(e) => setHorarioSaida(e.target.value)}
              />
              <small>Deixe em branco para manter o registro em aberto.</small>
            </div>
          </div>

          <div className="actions-row">
            <button className="btn-primary" onClick={salvarEntradaOuSaida}>
              Salvar entrada / saída
            </button>
            <button className="btn-secondary" onClick={limparFormulario}>
              Limpar
            </button>
          </div>
        </section>

        <section className="card">
          <h2>Consulta</h2>

          <div className="filters-grid">
            <div className="field">
              <label>Buscar por nome</label>
              <input
                type="text"
                placeholder="Digite o nome"
                value={buscaNome}
                onChange={(e) => setBuscaNome(e.target.value)}
              />
            </div>

            <div className="field">
              <label>Buscar por data</label>
              <input
                type="date"
                value={buscaData}
                onChange={(e) => setBuscaData(e.target.value)}
              />
            </div>
          </div>
        </section>

        <section className="card">
          <h2>Registros</h2>

          {loadingRegistros ? (
            <p>Carregando registros...</p>
          ) : registrosFiltrados.length === 0 ? (
            <p className="empty">Nenhum registro encontrado.</p>
          ) : (
            <div className="table-wrapper">
              <table>
                <thead>
                  <tr>
                    <th>Crachá</th>
                    <th>Nome</th>
                    <th>Box Acautelou</th>
                    <th>Entrada</th>
                    <th>Saída</th>
                    <th>Status</th>
                  </tr>
                </thead>
                <tbody>
                  {registrosFiltrados.map((item) => (
                    <tr key={item.id}>
                      <td>{item.numero_cracha}</td>
                      <td>{item.nome}</td>
                      <td>{item.box_acautelou}</td>
                      <td>{formatarDataHora(item.horario_entrada)}</td>
                      <td>
                        {item.horario_saida
                          ? formatarDataHora(item.horario_saida)
                          : '-'}
                      </td>
                      <td>
                        {item.horario_saida ? (
                          <span className="badge closed">Fechado</span>
                        ) : (
                          <span className="badge open">Em aberto</span>
                        )}
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
          )}
        </section>
      </div>
    </div>
  );
}

function formatarDataHora(valor: string) {
  const data = new Date(valor);

  return data.toLocaleString('pt-BR', {
    dateStyle: 'short',
    timeStyle: 'short',
  });
}
